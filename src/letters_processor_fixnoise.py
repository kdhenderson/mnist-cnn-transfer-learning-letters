...
class LetterProcessor:
    ...
    def detect_paper(self):
        ...
        # Step 1: Convert to grayscale (simplifies processing, removes color info)
    ...
    def save_letters(self):
        ...
        if not self.letter_label:
            ...
            return
        # Create output directory structure for saving processed letters
        ...
        # Convert to grayscale for processing (most image processing works on single channel)
        if len(self.processed_image.shape) == 3:
            gray = cv2.cvtColor(self.processed_image, cv2.COLOR_BGR2GRAY)
        else:
            gray = self.processed_image.copy()
        # Prepare CSV data for indexing saved letters
        csv_data = []
        ...
        # Process each bounding box (each detected letter)
        for i, (x, y, w, h) in enumerate(self.bounding_boxes):
            # Extract the letter region from the grayscale image
            letter_img = gray[y:y+h, x:x+w]
            # Skip if the region is empty (can happen if box is out of bounds)
            if letter_img.size == 0:
                print(f"Warning: Empty region for box {i}. Skipping.")
                continue
            # --- Thresholding: separate letter from background ---
            ...
            # Adaptive thresholding: computes a local threshold for each region (good for uneven lighting)
            if letter_img.shape[0] > 11 and letter_img.shape[1] > 11:
                thresh_adaptive = cv2.adaptiveThreshold(
                    letter_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                    cv2.THRESH_BINARY_INV, 11, 2
                )
                # Combine both methods to get the best of both worlds
                thresh = cv2.bitwise_or(thresh_otsu, thresh_adaptive)
            else:
                thresh = thresh_otsu
            # --- Noise Reduction: Erosion ---
            # Erosion shrinks white regions, helping to eliminate small specks (noise) inside the letter
            # (3,3) kernel, 1 iteration: balances noise removal and letter preservation
            kernel_erode = np.ones((3, 3), np.uint8)
            eroded = cv2.erode(thresh, kernel_erode, iterations=1)
            # --- Morphological Opening: Remove small white noise ---
            # Opening = Erosion followed by Dilation; removes small white blobs
            kernel = np.ones((2, 2), np.uint8)
            cleaned = cv2.morphologyEx(eroded, cv2.MORPH_OPEN, kernel)
            # --- Further Opening: Remove even more small noise ---
            # Larger kernel (3,3) for a second pass
            kernel_noise = np.ones((3, 3), np.uint8)
            cleaned = cv2.morphologyEx(cleaned, cv2.MORPH_OPEN, kernel_noise)
            # --- Contour Filtering: Keep only the largest contour (the main letter) ---
            # This removes any remaining small blobs inside the blank spaces (e.g., inside 'B', 'A')
            contours, _ = cv2.findContours(cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            if not contours:
                print(f"Warning: No contours found in box {i}. Using thresholded image directly.")
                clean_letter = cleaned
            else:
                # Find the largest contour (assumed to be the letter)
                letter_contour = max(contours, key=cv2.contourArea)
                # Create a mask for just the largest contour
                letter_mask = np.zeros_like(thresh)
                cv2.drawContours(letter_mask, [letter_contour], 0, 255, -1)
                # Keep only the pixels inside the largest contour
                clean_letter = cv2.bitwise_and(cleaned, letter_mask)
            # --- Padding: Center the letter in a square image ---
            # Add a border to ensure the letter is not touching the edge
            padding = 5
        ...
        # --- Save CSV index for all processed letters ---
        if csv_data:
