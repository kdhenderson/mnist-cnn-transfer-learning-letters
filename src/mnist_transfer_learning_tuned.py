#!/usr/bin/env python3
# MNIST Transfer Learning for digits 5-9 (Fine-tuned version)
# Uses a pre-trained CNN model on digits 0-4 and applies transfer learning with optimized hyperparameters
...
def load_base_model(model_path):
    ...
    # FINE-TUNING: Reduced dropout rates for better training accuracy
    transfer_model.add(layers.Dense(256, activation='relu', name='transfer_dense_1'))
    transfer_model.add(layers.Dropout(0.3, name='transfer_dropout_1'))  # Reduced from 0.5 to 0.3
    transfer_model.add(layers.Dense(128, activation='relu', name='transfer_dense_2'))
    transfer_model.add(layers.Dropout(0.2, name='transfer_dropout_2'))  # Reduced from 0.3 to 0.2
    ...
    # Compile the model with a reduced learning rate for more stable convergence
    # FINE-TUNING: Reduced learning rate from 0.001 to 0.0005
...
def plot_training_history(history, model, figures_dir, prefix="transfer_tuned_"):
    """
...
def save_training_results(history, model, results_dir, x_train, x_test, prefix="transfer_tuned_"):
    """
...
def train_and_evaluate_model(model, x_train, y_train, x_test, y_test, batch_size=64, epochs=50):
    ...
    """
    # FINE-TUNING: Increased patience from 3 to 5 for better convergence
    early_stopping = EarlyStopping(
        monitor='val_loss',      # Monitor validation loss
        patience=5,              # Wait for 5 epochs without improvement (increased from 3)
        restore_best_weights=True,  # Restore weights from best epoch
        verbose=1                # Print messages when stopping early
    )
    
    # FINE-TUNING: Added learning rate scheduler to reduce LR when plateauing
    reduce_lr = ReduceLROnPlateau(
        monitor='val_loss',
        factor=0.5,              # Reduce LR by half when plateauing
        patience=3,              # Wait for 3 epochs before reducing LR
        min_lr=0.00001,          # Don't reduce LR below this value
        verbose=1
    )
...
def main():
    ...
    print("Loading and modifying pre-trained model for transfer learning (with fine-tuned hyperparameters)...")
    ...
    print("\nTraining only the final layers for digits 5-9...")
    ...
    # Save the model
    model_path = os.path.join(results_dir, f"mnist_transfer_tuned_model_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}")
